...
contract uniLiquidity{
    ...
    function addLiquidityOneClickETH(address _token0, address _token1) public payable{
        ...
        if(_token0==UNIROUTERv2.WETH()){
            ...
            uint[] memory amounts1 = UNIROUTERv2.swapExactETHForTokens{value:msg.value/2}(0, path2, address(this), block.timestamp+180);
            
            IERC20(_token1).approve(address(UNIROUTERv2),amounts1[1]);
            
            (uint amountA, uint amountB, uint liquidity) = UNIROUTERv2.addLiquidityETH{value:msg.value/2}(_token1, amounts1[1], 0 ,0, msg.sender, block.timestamp + 180);
             
            if(amounts1[1]>amountA){
                IERC20(_token1).transfer(msg.sender, amounts1[1]-amountA);
            }
            if(msg.value>amountB){
                payable(msg.sender).transfer((msg.value/2)-amountB); 
            } 
        }
        else{
            ...
            uint[] memory amounts0 = UNIROUTERv2.swapExactETHForTokens{value:msg.value/2}(0, path1, address(this), block.timestamp+180);
            uint[] memory amounts1 = UNIROUTERv2.swapExactETHForTokens{value:msg.value/2}(0, path2, address(this), block.timestamp+180);
