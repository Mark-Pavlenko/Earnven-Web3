...
class Swapping extends Component {
  ...
  async onApprove(i) {
    ...
    await this.state.erc20.methods
      ...
      .then((balance) => {
        ...
        if (
          parseInt(balance) > parseInt(tokenBlock.tokenAmount) &&
          amount <= tokenBlock.tokenAmount
        ) {
          this.state.erc20.methods
            .symbol()
            .call({ from: this.state.account })
            ...
            .catch(console.log);
          this.state.erc20.methods
            .approve('0x868501a0c7743624b3C7Cb33C36A1E929Ab8472E', tokenBlock.tokenAmount)
  ...
  swapToken() {
    ...
    if (isValid) {
      ...
      this.state.swapping.methods
        .swap(tokenInputAmount, tokenInputAddress, tokenPath, isEth, targetTokenAddress)
        .send({ from: this.state.account });
    } else {
                  ...
                  <div className="col-sm-10">
                    <select
                      ...
                      {this.state.options.map((value, label) => {
                        return (
                          <option value={this.state.options[label].value}>
                            {this.state.options[label].label}
                          </option>
                        );
                      })}
                    </select>
