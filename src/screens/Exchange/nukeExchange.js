    ...
    if (TokenFrom.length > 0 && TokenTo !== '') {
      ...
      for (let i = 0; i < TokenFrom.length; i++) {
        if (parseFloat(TokenFrom[i].tokenAmount) > 0) {
          totalValue =
            totalValue +
            parseFloat(TokenFrom[i].tokenAmount) * parseFloat(TokenFrom[i].targetPrice);
        }
      }
            ...
            <Box sx={{ mt: 4, mb: 3 }}>
              {oneToMany === true ? (
                <Stack spacing={2}>
                  ...
                  <FiRepeat
                    style={{ marginLeft: '294px', marginTop: '20px', cursor: 'pointer' }}
                    onClick={() => reverse()}
                  />
              ...
              <Stack direction="row" spacing={1} sx={{ mt: 1.5 }}>
                ...
                <Typography variant="body2">
                  {totalToAmount !== ''
                    ? oneToMany == true
                      ? totalToAmount - totalToAmount * (Slippage / 100)
                      : totalToAmount
                    : '00.00'}
                  {TokenTo !== '' ? TokenTo.symbol : ''}
                </Typography>
              ...
              <Stack direction="row" spacing={1} sx={{ mt: 1.5 }}>
                ...
                <Stack>
                  {TokenFrom.map((token) => (
                    <Typography variant="body2">
                      {token.targetPrice !== undefined && TokenTo !== ''
                        ? `1${token.symbol}=${token.targetPrice}${TokenTo.symbol}`
                        : ''}
                    </Typography>
                  ))}
                </Stack>
