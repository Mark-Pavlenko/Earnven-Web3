...
export default function Exchange() {
  ...
  const [ContractAddress, setcontractAddress] = useState(
    '0xD23774726DB4d3D03Ba483514d7c8DF9bE729eEa'
  );
      ...
      if (TokenFrom !== '' && TokenTo !== '') {
        ...
        for (let i = 0; i < protocolsList.length; i++) {
          try {
            const protocolQuote = {};
            const response = await axios.get(
              `https://api.0x.org/swap/v1/quote?buyToken=${TokenTo.symbol}&sellToken=${TokenFrom}&sellAmount=${amount}&feeRecipient=0xE609192618aD9aC825B981fFECf3Dfd5E92E3cFB&buyTokenPercentageFee=0.02&includedSources=${protocolsList[i]}`
            );
  ...
  const getPrice = async (router, amount, firstpath, secondpath, decimals, route) => {
    ...
    try {
      const res = await router
        ...
        .call({ from: '0xD5Cd7dC05279653F960736482aBc7A7B2bF39B5d' });
  ...
  const handleMultiToSingleTokenSwap = async () => {
    ...
    if (isValid) {
      ...
      const tokenInputAddress = tokenBlock.map((token) =>
        !!token.address ? token.address : token.path == 1 ? weth : weth2
      );
  ...
  const tokenToTotalValue = async () => {
    if (TokenFrom.length > 0 && TokenTo !== '') {
      ...
      let totalValue = 0;
      for (let i = 0; i < TokenFrom.length; i++) {
        totalValue += parseFloat(TokenFrom[i].tokenAmount) * parseFloat(TokenFrom[i].targetPrice);
      }
            ...
            <Box sx={{ mt: 4, mb: 3 }}>
              <Stack spacing={2}>
                <Stack spacing={0.5}>
                  ...
                  <FiPlusCircle
                    style={{ marginLeft: '54px', cursor: 'pointer' }}
              ...
              <Stack direction="row" sx={{ mt: 2 }}>
                {Sources.map((object) => (
                  <div>
                    ...
                    <FaAngleRight
                      style={{ paddingTop: '6px', marginLeft: '1px', color: '#737373' }}
                    />
                  </div>
                ))}
              </Stack>
              ...
              <Stack direction="row" spacing={1} sx={{ mt: 1.5 }}>
                ...
                <Typography variant="body2">
                  {totalToAmount !== '' ? totalToAmount : '00.00'}
              ...
              <Stack direction="row" spacing={1} sx={{ mt: 1.5 }}>
                ...
                <Stack>
                  <Typography variant="body2">
                    {TokenFrom[0] !== undefined && TokenTo !== ''
                      ? `1${TokenFrom[0].symbol}=${TokenFrom[0].targetPrice}${TokenTo.symbol}`
                      : ''}
                  </Typography>
                  <Typography variant="body2">
                    {TokenFrom[1] !== undefined && TokenTo !== ''
                      ? `1${TokenFrom[1].symbol}=${TokenFrom[1].targetPrice}${TokenTo.symbol}`
                      : ''}
                  </Typography>
                  <Typography variant="body2">
                    {TokenFrom[2] !== undefined && TokenTo !== ''
                      ? `1${TokenFrom[2].symbol}=${TokenFrom[2].targetPrice}${TokenTo.symbol}`
                      : ''}
                  </Typography>
                </Stack>
              </Stack>
              ...
              <Box
                ...
                <Stack direction="row" spacing={2}>
                  ...
                  <Stack direction="column">
                    ...
                    <Typography variant="caption" sx={{ color: '#e3e3e3', fontSize: '11px' }}>
                      {object.name}
                    </Typography>
                  </Stack>
