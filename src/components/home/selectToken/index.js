...
class SelectToken extends Component {
  ...
  async mounted() {
    ...
    if (typeof this.state.ls.get('savedTokens') === 'object') {
      this.state.data = this.state.ls.get('savedTokens')[this.props.chainId]
        ? this.state.data.concat(this.state.ls.get('savedTokens')[this.props.chainId])
        : this.state.data;
      this.setState([...this.state.data]);
    }
  ...
  secureLsInit() {
    ...
    if (
      !this.state.ls.getAllKeys().includes('savedTokens') ||
      typeof this.state.ls.get('savedTokens') === 'string'
    ) {
      const obj = {};
    ...
    if (
      !this.state.ls.getAllKeys().includes('isDfynChecked') ||
      typeof this.state.ls.get('isDfynChecked') !== 'boolean'
    ) {
      this.state.ls.set('isDfynChecked', false);
    }
    if (
      !this.state.ls.getAllKeys().includes('isQuickswapChecked') ||
      typeof this.state.ls.get('isQuickswapChecked') !== 'boolean'
    ) {
      this.state.ls.set('isQuickswapChecked', false);
    }
    if (
      !this.state.ls.getAllKeys().includes('isSushiChecked') ||
      typeof this.state.ls.get('isSushiChecked') !== 'boolean'
    ) {
  ...
  search(text) {
    return (searchItem) => {
      if (
        searchItem.symbol.toLowerCase().includes(text.toLowerCase()) ||
        (text.length > 30 && searchItem.address.toLowerCase().includes(text.toLowerCase()))
      )
        return searchItem;
    };
  }
  ...
  async getSearchedToken(address) {
    ...
    await ercContract.methods
      ...
      .then(async (res) => {
        ...
        const obj = {
          logoURI:
            'https://cdn.imgbin.com/5/7/6/imgbin-computer-icons-question-mark-symbol-symbol-ENabv6w1xfvGrpWTRw4YrX25q.jpg',
          symbol: res,
          name: tokenName,
          address,
          decimals,
      ...
      .catch((error) => {
        ...
        this.setState({
          errorMessage:
            'The token you are trying to search may not be available in this network. Please check your token address carefully.',
        });
  ...
  importToken(tokenData) {
    const savedTokens = this.state.ls.get('savedTokens');
    if (
      Object.keys(savedTokens).includes(String(this.props.chainId)) &&
      savedTokens[this.props.chainId].length
    ) {
  ...
  async handleSelectToken(item) {
    if (!this.state.allSelectedTokens.includes(item.symbol)) {
      ...
      await this.props.handleFlipToken(
        tokenList[this.props.chainId].find((token) => token.symbol === this.props.currentToken),
        true,
        undefined,
        item.address
      );
          ...
          <div className="token-selector-container">
            <div className="token-selector-wrapper">
              <div className="select-token-header">
                <div className="select-token-text">Select a token</div>
                <svg
              ...
              <div className="common-base-container">
                ...
                <div className="common-base-tokenlist-container">
                  {this.state.commonBases.map((token) => {
                    return (
                      <div
                        className="common-base-token-container"
                        onClick={() => this.handleSelectToken(token)}>
                        <div className="common-base-token-wrapper">
                          <img className="coin-img" alt="" src={token.logoURI} />
                          <div className="coin-name">{token.symbol}</div>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="list-container">
                ...
                {this.state.data.length > 0 &&
                  this.state.filteredData.map((item, index) => {
                    return (
                      <div key={index} onClick={() => this.handleSelectToken(item)}>
                        <Currency src={item.logoURI} name={item.symbol} desc={item.name} />
                      </div>
                    );
                  })}
              ...
              <div className="manage-token-btn-container">
                <button
                  className="manage-token-btn"
                  onClick={() => this.setState({ isManage: true })}>
                  Manage Tokens
                </button>
              </div>
            </div>
          ...
          <div className="token-selector-container">
            <div className="token-selector-wrapper">
              ...
              <div className="import-details">
                ...
                <div className="token-ticker">{this.state.searchedData.symbol}</div>
                <div className="token-name">{this.state.searchedData.name}</div>
                <div className="token-address">{this.state.searchedData.address}</div>
                <div className="warning-container">
                  <div className="warning">
                    ...
                    <span className="warning-unknown">Unknown Sources</span>
                  </div>
                </div>
              </div>
              <button
                className="import-button"
                onClick={() => this.importToken(this.state.searchedData)}>
                Import
              </button>
            </div>
          </div>
              ...
              <div className="manage-token-wrapper">
                <div className="row lists-tokens-tab-header">
                  <div className="col-sm-6 list-tokens-container">
                    <button
                      className={`lists-tokens-tab ${
                        !this.state.isTabClicked ? 'active-tab' : 'inactive-tab'
                      }`}
                  ...
                  {!this.state.isTabClicked ? (
                    ...
                    <div>
                      <div className="custom-token-content-wrapper">
                        <div className="row custom-token-header">
                          <div className="col-sm-6">
                            <div className="custom-token-header-left">
                              {`${this.state.savedTokens.length} ${
                                this.state.savedTokens.length > 1 ? 'Custom Tokens' : 'Custom Token'
                              }`}
                            </div>
                          </div>
                          <div className="col-sm-6">
                        ...
                        {this.state.savedTokens.length > 0 &&
                          this.state.savedTokens.map((token, index) => {
                            return (
          ...
          <div className="token-selector-container">
            <div className="token-selector-wrapper">
              ...
              <div className="import-warning">
                ...
                <p style={{ fontSize: '18px' }}>{this.state.errorMessage}</p>
              </div>
            </div>
          </div>
        )}
