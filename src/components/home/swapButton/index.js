      ...
      <StyledSwapButtonContainer>
        {this.props.isMultiToSingleToken ? (
          <div className="connect-wallet-wrapper">
            {this.props.tokenBlock.every((token) => token.isApprove && token.tokenAmount > 0) &&
            !!this.props.selectedDownToken.name ? (
              <button className="connect-wallet-btn" onClick={() => this.props.handleSwapToken()}>
                Swap
              </button>
            ) : this.props.tokenBlock.every((token) => !!token.symbol) &&
              !!this.props.selectedDownToken.name &&
              ...
              !this.props.selectedDownToken.name ? (
              <button className="connect-wallet-btn">Select a token</button>
            ) : (
          ...
          <div className="connect-wallet-wrapper">
            ...
            this.props.tokenBlock.every((token) => token.tokenAmount > 0) &&
            this.props.tokenBlock.every((token) => !!token.name) ? (
              <button className="connect-wallet-btn" onClick={() => this.props.handleSwapToken()}>
                Swap
              </button>
