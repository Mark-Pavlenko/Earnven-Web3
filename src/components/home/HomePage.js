...
class HomePage extends Component {
  ...
  componentDidMount() {
    ...
    if (
      !this.state.ls.getAllKeys().includes('approvedTokens') ||
      typeof this.state.ls.get('approvedTokens') === 'string'
    ) {
      this.state.ls.set('approvedTokens', {});
    }
    if (
      !this.state.ls.getAllKeys().includes('inputTokens') ||
      typeof this.state.ls.get('inputTokens') === 'string'
    ) {
  ...
  handleWalletConnected() {
    ...
    if (this.state.ls.get('inputTokens').length) {
      const approvedTokens = this.state.ls.get('approvedTokens')[account + contractAddress];
      const inputTokens = this.state.ls.get('inputTokens');
  ...
  calculateTotalAmount2() {
    const tokenPrices = this.state.tokenBlock.map((x) =>
      parseFloat(!!x.tokenAmount ? x.tokenAmount : 0)
    );
    const totalPrice = tokenPrices.reduce((x, y) => x + y).toFixed(6);
    this.setState({ secondInput: totalPrice });
  }
  ...
  handlePercentageChange(value, i) {
    ...
    if (parseInt(tokenInfo.balance) > 100000) {
      const amount =
        (((parseInt(tokenInfo.balance) - 100000) / 10 ** tokenInfo.decimals) * value) / 100;
  ...
  async handleUpperSelectToken(item, isFlipped = false, index = this.state.selectedIndex, address) {
    const tokenInfo = this.state.tokenBlock[index];
    ...
    if (!!item) {
      ...
      for (let i = 0; i < tokenBlock.length; i++) {
        const token = tokenBlock[i];
        if (token.address === address) {
          tokenBlock[i] = initialState;
        }
      }
      this.setState({ tokenBlock });
  ...
  async updateInputBalances(account, web3) {
    const inputTokens = this.state.tokenBlock;
  ...
  async updateOutputBalance(account, web3) {
    ...
    const outputToken = Object.keys(savedOutputToken).length
      ? savedOutputToken
      : this.state.selectedDownToken;
  ...
  async updatePrice(
    ...
    if (
      ...
      if (outputTokenAddress !== wmatic && outputTokenAddress !== wmatic2) {
        // Dfyn Initial
        initialData = await this.getPrice(
          dfyn.methods,
          inputAmount,
          inputTokenAddress,
          wmatic,
  ...
  async getPrice(router, amount, firstPath, secondPath, decimals, route) {
    try {
      const res = await router
        .getAmountsOut(amount, [firstPath, secondPath])
  ...
  handleSwapToken() {
    ...
    if (
      tokenBlock.some(
        (token) =>
          parseFloat(token.tokenAmount) * 10 ** parseInt(token.decimals) > parseInt(token.balance)
      )
    ) {
  ...
  async handleMultiToSingleTokenSwap() {
    ...
    if (isValid) {
      ...
      const tokenInputAddress = tokenBlock.map((token) =>
        !!token.address ? token.address : token.path == 1 ? wmatic : wmatic2
      );
      const tokenInputAmount = tokenBlock.map((token) =>
        (parseFloat(token.tokenAmount) * 10 ** parseInt(token.decimals)).toString()
      );
      ...
      if (selectedDownToken.symbol === 'MATIC') {
        await this.state.swap.methods
          .swapToETH(tokenInputAmount, tokenInputAddress, swapRoute)
          .send({ from: account })
        ...
        const maticAmount = (
          parseFloat(matic.tokenAmount) *
          10 ** parseInt(matic.decimals)
        ).toString();
        await this.state.swap.methods
          .swap(
            tokenInputAmount,
            ...
            isEth,
            targetTokenAddress,
            isMultiToSingleToken,
            selectedDownToken.isEth
          )
          .send({ from: account, value: maticAmount })
          .once('transactionHash', (txHash) => {
            this.setState({ transactionHash: txHash });
            this.setState({ isSwapLoading: false });
            this.setState({ isTransactionSubmitted: true });
  ...
  async handleSingleToMultiTokenSwap() {
    ...
    if (isValid) {
      ...
      const tokenInputAmount = tokenBlock.map((token) =>
        (parseFloat(token.tokenAmount) * 10 ** parseInt(selectedDownToken.decimals)).toString()
      );
      const swapRoute = tokenBlock.map((token) => token.path);
      const isEth = tokenBlock.map((token) => token.isEth);
      const targetTokenAddress = selectedDownToken.address;
      ...
      if (selectedDownToken.symbol === 'MATIC') {
        const maticAmount = (
          parseFloat(this.state.secondInput) *
          10 ** parseInt(selectedDownToken.decimals)
        ).toString();
        await this.state.swap.methods
          .swap(
            tokenInputAmount,
  ...
  async handleSwapAllTokens() {
    ...
    const approvedTokenList = tokenList[chainId].filter(
      (token) =>
        (approvedTokens.includes(token.address) &&
          token.address !== this.state.defaultToken.address) ||
        (token.symbol === 'MATIC' && !token.address)
    );
    const outputToken = tokenList[chainId].find(
      (token) => token.address === this.state.defaultToken.address
    );
    this.handleDownSelectToken(outputToken);
    ...
    for (let i = 0; i < approvedTokenList.length; i++) {
      ...
      if (token.balance <= 0 || (token.symbol === 'MATIC' && parseInt(token.balance) <= 1e18)) {
        ...
        this.handleInput1Change(
          (parseInt(token.balance) - 1e18) / 10 ** token.decimals,
          i - invalidCount
        );
      } else {
        this.handleInput1Change(
          (parseInt(token.balance) - 100000) / 10 ** token.decimals,
          i - invalidCount
        );
      }
    }
    ...
    return (
      <ThemeProvider theme={this.state.theme === 'light' ? lightTheme : darkTheme}>
        ...
        <StyledHomePageContainer>
          {this.state.isNukeInvoked && <div className="top-layer" />}
          {/*  <Navbar 
                    ...
                    {this.state.isMultiToSingleToken ? (
                      <div>
                        <MultiTokenContainer
                          ...
                          handlePercentageChange={(value, i) =>
                            this.handlePercentageChange(value, i)
                          }
                          handleTokenUpperDropdownClicked={(i) =>
                            this.handleTokenUpperDropdownClicked(i)
                          }
                        />
                        <TokenAddAndChangeContainer
                          componentType="add & change"
                          handleAddInput={() => this.handleAddInput()}
                          toggleMultiToSingleToken={() =>
                            this.handleTransition(this.state.isMultiToSingleToken)
                          }
                        />
                        <SingleTokenContainer
                          ...
                          handleTokenDownDropdownClicked={() =>
                            this.handleTokenDownDropdownClicked()
                          }
                        />
                      </div>
                    ) : (
          ...
          {/* Modals Section */}
          <Modal
            toggle={() => this.setState({ tokenUpperDropdownClicked: false })}
            open={this.state.tokenUpperDropdownClicked}>
            <SelectToken
              theme={this.state.theme}
              ...
              isTargetToken={false}
              currentToken={
                this.state.tokenBlock.length > this.state.selectedIndex
                  ? this.state.tokenBlock[this.state.selectedIndex].symbol
                  : undefined
              }
          ...
          <Modal
            toggle={() => this.setState({ tokenDownDropdownClicked: false })}
            open={this.state.tokenDownDropdownClicked}>
            <SelectToken
              theme={this.state.theme}
          ...
          <Modal
            open={this.state.isSwapLoading}
            toggle={() => this.setState({ isSwapLoading: false })}>
            <ConfirmSwap
              theme={this.state.theme}
          ...
          <Modal
            open={this.state.isTransactionSubmitted}
            toggle={() => this.setState({ isTransactionSubmitted: false })}>
