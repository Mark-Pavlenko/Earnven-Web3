...
const PoolDetailChart = (props) => {
  ...
  const getMonths = () => {
    ...
    var offset = new Date(year, month, day).getTimezoneOffset() * 60
    var epoch = new Date(year, month, day).getTime() / 1000 - offset
    var currentDate = new Date(epoch * 1000)
  ...
  useEffect(() => {
    ...
    async function getData() {
      ...
      //call this util function to get uniswap graph data
      const response = await getUniswapGraphData(tokenPair, twoMonthPrior)
      //console.log('uniswap data', response)
  ...
  useEffect(() => {
    ...
    async function getData() {
      ...
      //0x0d4a11d5eeaac28ec3f61d100daf4d40471f1852 - weth/usdt
      //0xa478c2975ab1ea89e8196811f51a7b7ade33eb11
      const response = await getUniswapGraphData(tokenPair, threeMonthPrior)
      //console.log('uniswap data', response)
