...
export default function LiquidityPools() {
            ...
            await axios.get(`https://tokens.coingecko.com/uniswap/all.json`, {}, {})
                .then(async (response) => {
                    ...
                    let tokens = fetchedTokens.map((token) => ({
                        ...token,
                        logoURI: data.find(x => x.address == token.address) ? data.find(x => x.address == token.address).logoURI : tokenURIs.find(x => x.address == token.address).logoURI
                    }));
    ...
    useEffect(() => {
                    ...
                    >
                        <div style={{height: 'auto', minHeight: '60px', width: '100%'}}>
                            ...
                            <div style={{display: 'inline-block', width: '30%', marginTop: '10px'}}>
                                ...
                                {((parseInt(object.volumeUSD) * 0.003) / parseInt(object.reserveUSD) * 100 * 365).toFixed(2)} %
                                (Yearly)<br/>
                                {((parseInt(object.volumeUSD) * 0.003) / parseInt(object.reserveUSD) * 100 * 7).toFixed(2)} %
                                (Weekly)
                            </div>
                            <div style={{display: 'inline-block', width: '15%',}}>
                                {
                                    object.tokens.map((tok) =>
                                        <>
                                            <img style={{height: '30px', width: '30px', display: 'inline-block'}} alt=''
                                                 src={`https://ethplorer.io${tok.image}`}/>
                                            &nbsp;
                                        </>
                                    )
                                }
                            ...
                            <center>
                                <AppBar position="static" style={{width: '400px'}}>
                                    <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
                                        <Tab className={classes.root} style={{width: '50%'}}
                                             label="Add Liquidity" {...a11yProps(0)} />
                                        <Tab className={classes.root} style={{width: '50%'}}
                                             label="Remove Liquidity" {...a11yProps(1)} />
                                    </Tabs>
                                </AppBar>
                            </center>
        ...
        >
                    ...
                    {AllTokens[4].symbol} */}
                                        ...
                                        <TransparentButton
                                            onClick={(e) => {
                                                addLiquidity(object.token0.id, object.token1.id, SupplyToken, (SupplyTokenAmount * (10 ** 18)).toString())
                                            }}
        ...
        >
                                ...
                                <center> OR <br/><br/><br/>
                                    ...
                                    <TransparentButton
                                        onClick={(e) => {
                                            addLiquidityNormal(object.token0.id, object.token1.id, AmountTokenA, AmountTokenB)
                                        }}
                            ...
                            <TabPanel value={value} index={1}>
                                <center>
                                    <br/><br/>
                                    Liquidity To Remove &nbsp; : &nbsp; <AmountInput onChange={(e) => {
                                    setLiquidityAmount(e.target.value);
                                    checkLiquidity(object.token0.id, object.token1.id)
                                }}/>
                                    <br/>
                                    <font style={{fontSize: '10px'}}>Liquidity In
                                        Account &nbsp;: {AccountLiquidity}  </font>
                                    <br/><br/>
                ...
                style={{background:'transparent',
                    ...
                    placeholder='Receive Token Address'/>  */}
                                        ...
                                        <TransparentButton value="Remove Liquidity In Tokens"
                                                           onClick={(e) => {
                                                               removeLiquidity(object.token0.id, object.token1.id, ReceiveToken, (LiquidityAmount * (10 ** 18)).toString())
                                                           }}
                                        />
                                    </div>
                                    OR
                                    <div style={{display: 'inline-block', width: '40%'}}>
                                        <TransparentButton value="Remove Liquidity in Ether"
                                                           onClick={(e) => {
                                                               removeLiquidityETH(object.token0.id, object.token1.id, (LiquidityAmount * (10 ** 18)).toString())
                                                           }}
                                        />
                                    </div>
                                    <br/><br/> <b>OR</b> <br/><br/>
                                    ...
                                    <TransparentButton value="Remove Liquidity Classic"
                                                       onClick={(e) => {
                                                           removeLiquidityNormal(object.token0.id, object.token1.id, (LiquidityAmount * (10 ** 18)).toString())
                                                       }}
                                    />
            ...
            await axios.post(`https://api.thegraph.com/subgraphs/name/balancer-labs/balancer`,
                ...
                .then(async (response) => {
                    ...
                    if (response.data.data) {
                        var res = response.data.data.pools
                        for (var i = 0; i < res.length; i++) {
                            res[i].token0 = 'haha'
                            res[i].token1 = 'haha'
                            for (var j = 0; j < res[i].tokens.length; j++) {
                                // console.log('lul')

                                await axios.get(`https://api.ethplorer.io/getTokenInfo/${res[i].tokens[j].address}?apiKey=EK-qSPda-W9rX7yJ-UY93y`, {}, {})
                                    .then((response) => {
    ...
    async function addLiquidityEth(tokenA, tokenB, ethAmount) {
        ...
        const accounts = await web3.eth.getAccounts();
        const oneClickContract = new web3.eth.Contract(OneClickLiquidity, Addresses.oneClickSushiV2Contract);
        await oneClickContract.methods.addLiquidityOneClickETH(tokenA, tokenB).send({
            from: accounts[0],
