...
export default function LiquidityPools() {
      ...
      await axios
        ...
        .then(async (response) => {
          ...
          setAllTokens(tokens);
            ...
            console.log('liquidity pools Uniswap-V3 returns error', res)});
    }
    getData();
  }, []);
          ...
          <AccordionSummary
            ...
            <div style={{ height: '60px', width: '100%' }}>
              ...
              <div style={{ display: 'inline-block', width: '30%', marginTop: '10px' }}>
                {(
                  ((parseInt(object.volumeUSD) * 0.003) / parseInt(object.tvlUSD)) *
                  100 *
                  365
                ).toFixed(2)}{' '}
                % (Yearly)
              ...
              <div style={{ display: 'inline-block', width: '10%' }}>
                <img
                  ...
                  alt=""
                  src={`https://ethplorer.io${object.pool.token0.image}`}
                />
                &nbsp;&nbsp;&nbsp;
                <img
                  style={{ height: '30px', width: '30px', display: 'inline-block' }}
                  alt=""
                  src={`https://ethplorer.io${object.pool.token1.image}`}
                />
              </div>
            </div>
          </AccordionSummary>
  ...
  useEffect(() => {
    ...
    async function getData() {
      ...
      await axios
        .post(
          `https://gateway.thegraph.com/api/${Addresses.graph_API}/subgraphs/id/0x9bde7bf4d5b13ef94373ced7c8ee0be59735a298-2`,
          {
            query: `{
                    poolDayDatas
                    (
        ...
        .then(async (response) => {
          ...
          if (response.data.data) {
            ...
            for (var i = 0; i < res.length; i++) {
              await axios
                .get(
                  `https://api.ethplorer.io/getTokenInfo/${res[i].pool.token0.id}?apiKey=EK-qSPda-W9rX7yJ-UY93y`,
                  {},
                  {}
                )
                .then((response) => {
              ...
              await axios
                .get(
                  `https://api.ethplorer.io/getTokenInfo/${res[i].pool.token1.id}?apiKey=EK-qSPda-W9rX7yJ-UY93y`,
                  {},
                  {}
                )
                .then((response) => {
