          ...
          <AccordionSummary
            ...
            <div style={{ height: 'auto', minHeight: '60px', width: '100%' }}>
              ...
              <div style={{ display: 'inline-block', width: '30%', marginTop: '10px' }}>
                {(
                  ((parseInt(object.volume) * object.pool.fee) / parseInt(object.pool.liquidity)) *
                  100 *
                  365
                ).toFixed(2)}{' '}
                % (Yearly)
          ...
          if (response.data.data) {
            ...
            for (var i = 0; i < res.length; i++) {
              ...
              for (var j = 0; j < res[i].pool.underlyingCoins.length; j++) {
                ...
                await axios
                  .get(
                    `https://api.ethplorer.io/getTokenInfo/${res[i].pool.underlyingCoins[j].token.address}?apiKey=EK-qSPda-W9rX7yJ-UY93y`,
                    {},
                    {}
                  )
                  .then((response) => {
                    ...
                    if (response.data.image) {
                      ...
                      if (res[i].pool.underlyingCoins[j].balance < 1) {
                        res[i].pool.underlyingCoins[j].balance =
                          res[i].pool.underlyingCoins[j].balance * 10 ** 12;
                      }
                      liquidity +=
                        parseFloat(response.data.price.rate) *
                        parseFloat(res[i].pool.underlyingCoins[j].balance);
                    }
                  })
                  .catch((err) => {
                    console.log(err);
