...
class Settings extends Component {
  ...
  secureLsInit() {
    ...
    if (
      !this.state.ls.getAllKeys().includes('defaultToken') ||
      typeof this.state.ls.get('defaultToken') === 'string'
    ) {
      this.state.ls.set('defaultToken', this.state.defaultToken);
    } else if (
      this.state.ls.getAllKeys().includes('defaultToken') &&
      Object.keys(this.state.ls.get('defaultToken')).length
    ) {
              ...
              <div className="expert-multihops-wrapper">
                <div className="expert-multihops-header">
                  ...
                  <Dropdown className="inline-block">
                    ...
                    <Dropdown.Menu>
                      {this.state.stableTokens
                        .filter((token) => token.address !== this.state.defaultToken.address)
                        .map((token) => {
                          return (
                            <Dropdown.Item onClick={() => this.changeDefaultToken(token)}>
                              <div className="img-text-wrapper">
                                <img
                                  ...
                                  className="coin-image"
                                />
                                <span className="token-symbol-container">{token.symbol}</span>
                              </div>
                            </Dropdown.Item>
                          );
                        })}
